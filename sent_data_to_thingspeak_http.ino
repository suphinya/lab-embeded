...
// ThingSpeak Settings
String apiKey = "";
String serverName = "https://api.thingspeak.com/update?api_key="+apiKey;
...
void loop() {
  ...
  analog_value = analogRead(analogpin); // LDR
  ...
  if ((millis() - lastTime) > timerDelay) {
    ...
    if(WiFi.status()== WL_CONNECTED){
      ...
      // request to thingspeak
      http.begin(serverName);
      http.addHeader("Content-Type", "application/x-www-form-urlencoded");
      // Data to send with HTTP POST
      String httpRequestData = "api_key=" + apiKey + "&field1=" + String(value) + "&field2=" + String(analog_value) ;
